...
+ (void)showSaidHi
{
    [self showImage:[UIImage imageNamed:@"said_hi_success"] status:@"发送成功"];
}

+ (void)showIncognito:(NSString*)message
{
    [self showImage:[UIImage imageNamed:@"incognito"] status:message];
}
...
{
    [self sharedView].maskType = SVProgressHUDMaskTypeNone;
    [[self sharedView] showImage2:nil status:string duration:displayInterval];
}
...
{
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(actuallyDismiss) object:nil];
    if(!self.overlayView.superview){
        NSEnumerator *frontToBackWindows = [[[UIApplication sharedApplication]windows]reverseObjectEnumerator];
        
        //[AppDelegateInstance.window addSubview:self.overlayView];
        
        for (UIWindow *window in frontToBackWindows){
            //if (window.windowLevel != UIWindowLevelStatusBar && window.windowLevel != UIWindowLevelAlert)
            //{
            if ([window isEqual:(AppDelegate*)[[UIApplication sharedApplication] delegate].window]) {
...
- (void)showImage:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    ...
    if(![self.class isVisible])
        [self.class showWithStatus:nil maskType:self.maskType];
...
- (void)showImage2:(UIImage *)image status:(NSString *)string duration:(NSTimeInterval)duration {
    [NSObject cancelPreviousPerformRequestsWithTarget:self selector:@selector(actuallyDismiss) object:nil];
    self.progress = -1;
    [self cancelRingLayerAnimation];
...
- (void)dismiss
{
    [self performSelector:@selector(actuallyDismiss) withObject:nil afterDelay:0];
}

- (void)actuallyDismiss
{
    NSDictionary *userInfo = [self notificationUserInfo];
...
//        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 28, 28)];
        ...
        imageView = [[UIImageView alloc] initWithFrame:CGRectMake(0, 0, 39, 39)];//by patrick
