...
- (NSString*)cachedFilePathForURL:(NSURL*)sourceURL
{
...
//            NSLog(@"#### %s, %u", entry->fts_path, entry->fts_statp->st_size); // DEBUG
            ...
            if (unlink(entry->fts_path)) {
                NSLog(@"%s|[ERROR] failed to remove %s",
...
{
    NSString* cachedFilePath = [self cachedFilePathForURL:sourceURL];
    NSFileManager* fileManager = [NSFileManager defaultManager];
    NSError* error = nil;
...
{
    FBCachedFile* cachedFile =
        [FBCachedFile cachedFile:[self cachedFilePathForURL:sourceURL]];
    self.fetchedCounter++;
    if (cachedFile) {
        self.hitCounter++;
    }
...
- (void)removeCachedFileForURL:(NSURL*)sourceURL
{
    ...
    if (stat([cachedFilePath UTF8String], &fileStat) == -1) {
        ...
        NSUInteger removingFileSize = (NSUInteger)fileStat.st_size;
